...
public class ProductDao {
	...
	//用于搜索
	public int getCount2(String parameter) throws SQLException {
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
		String sql = "select count(*) from product where pname like \"%\"?\"%\"";
		Long query = (Long) runner.query(sql, new ScalarHandler(),parameter);
		return query.intValue();
	}
    ...
    //用于搜索
    public List<Product> searchProducts(String parameter, int index, int currentCount) throws SQLException {
        QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
        String sql = "select * from product where pname like \"%\"?\"%\" limit ?,?";
        List<Product> list = runner.query(sql, new BeanListHandler<Product>(Product.class), parameter,index,currentCount);
        return list;
    }
