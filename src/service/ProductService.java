...
public class ProductService {
	...
	public PageBean searchProducts(String parameter,int currentPage,int currentCount) {
		...
		int totalCount = 0;
		try {
			totalCount = dao.getCount2(parameter);
		} catch (SQLException e) {
			e.printStackTrace();
		}
		pageBean.setTotalCount(totalCount);
		...
		List<Product> list = null;
		try {
			list = dao.searchProducts(parameter,index,currentCount);
		} catch (SQLException e) {
			e.printStackTrace();
		}
		pageBean.setList(list);
